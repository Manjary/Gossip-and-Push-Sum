defmodule Actor do
  ...
  #Set the state of neigh_list for an actor
  def set_neighbour(pid, neighbour_list) do
    GenServer.call(pid, {:neighbours, neighbour_list})
  end

  #Gets the neighbor list of an actor
  def get_neighbour(pid) do
    GenServer.call(pid, {:neighbours})
  end

  #Increments the counter of a receiver actor and sends the message
  ...
  #To calculate sender's [s,w] value
  ...
  def handle_cast({:pickNeigh,start_time},state) do
    ...
    if(count < 11 ) do
      ...
      GenServer.cast(nextNode,{:pickNeigh,start_time}) #Receiver Actor gossips with other neighbors 
      #IO.puts "calling my self"
      GenServer.cast(pid,{:pickNeigh,start_time}) #Sender Actor gossips with other neighbors, thereby introducing parallelism
      :timer.sleep(100)
  ...
  #Sets the sender's [s,w] values to half of its original values
  ...
  #Handling of pushSum
  #For all the actors that has not reached the threshold value, this method
  #adds the [s,w] value of sender to the receiver's [s,w] values.
  ...
  #Selects the reciever actor randomly from sender's neighbor list
  #and propagates the [s,w] values for active receiver actor.  
...
#Supervisor to initiate the actors via Genserver
